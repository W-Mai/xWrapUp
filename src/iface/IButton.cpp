...
ErrorCode IButton::getAttr(IDType id, void *ret_val, VAR_LIST(vars)) {
    auto func = FETCH_ATTR_FUNC(get);
    if (func == nullptr) return IObjBase::getAttr(id, ret_val, vars);
    return (this->*(func))(ret_val, vars);
}

ErrorCode IButton::setAttr(IDType id, void *ret_val, VAR_LIST(vars)) {
    auto func = FETCH_ATTR_FUNC(set);
    if (func == nullptr) return IObjBase::setAttr(id, ret_val, vars);
    return (this->*(func))(ret_val, vars);
}

ErrorCode IButton::exec(IDType id, void *ret_val, VAR_LIST(vars)) {
    auto func = FETCH_EXEC_FUNC();
    if (func == nullptr) return IObjBase::exec(id, ret_val, vars);
    return (this->*(func))(ret_val, vars);
}

ErrorCode IButton::getTest(int *ret_val, VAR_LIST(vars)) {
    *ret_val = width;
    return ErrorCode::OK;
}

ErrorCode IButton::setTest(void *ret_val, VAR_LIST(vars)) {
    width = VAR_GET(vars, int);
    return ErrorCode::OK;
}
